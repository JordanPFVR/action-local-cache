  ...
  my_job:
    ...
    steps:
      ...
      - name: Local cache for API dependencies
        id: api-cache
        uses: JordanPFVR/action-local-cache@2
        with:
...
For example, when running the usage example above inside a workflow for the `JordanPFVR/product` repo, this empty folder will be created:

```shell
~/runner/_work/_tool/JordanPFVR/product/api-dependencies-v1/
```
